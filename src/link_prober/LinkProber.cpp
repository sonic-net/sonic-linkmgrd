    ...
    if (!errorCode) {
        ...
        if (ntohl(icmpPayload->cookie) == IcmpPayload::getCookie() &&
            ...
            if (isMatch) {
                ...
                mReceivedHeartbeats.set(HeartbeatType::HEARTBEAT_SELF);
            } else {
                mRxPeerSeqNo = mTxSeqNo;
                mReceivedHeartbeats.set(HeartbeatType::HEARTBEAT_PEER);
            }
            mReportHeartbeatReplyReceivedFuncPtr();
...
//
void LinkProber::reportHeartbeatReplyReceivedActiveStandby()
{
...
//
// ---> reportHeartbeatReplyReceivedActiveActive
//
// report heartbeat reply not received to active-standby mode link prober state machine
//
void LinkProber::reportHeartbeatReplyReceivedActiveActive()
{
    if (mReceivedHeartbeats[HeartbeatType::HEARTBEAT_SELF] && mTxSeqNo == mRxSelfSeqNo) {
        mLinkProberStateMachinePtr->postLinkProberStateEvent(LinkProberStateMachineBase::getIcmpSelfEvent());
    }
    if (mReceivedHeartbeats[HeartbeatType::HEARTBEAT_PEER] && mTxSeqNo == mRxPeerSeqNo) {
