...
//
// ---> PeerDownState(LinkProberStateMachineBase &stateMachine, common::MuxPortConfig &muxPortConfig);
//
// class constructor
//
PeerDownState::PeerDownState(
...
//
LinkProberState* PeerDownState::handleEvent(IcmpPeerActiveEvent &event)
{
...
//
LinkProberState* PeerDownState::handleEvent(IcmpPeerUnknownEvent &event)
{
    MUXLOGDEBUG(getMuxPortConfig().getPortName());

    LinkProberStateMachineBase *stateMachine = dynamic_cast<LinkProberStateMachineBase *> (getStateMachine());
    LinkProberState *nextState = dynamic_cast<LinkProberState *> (stateMachine->getPeerDownState());
